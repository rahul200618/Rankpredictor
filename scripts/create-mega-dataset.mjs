...
function main() {
  ...
  console.log('Creating MEGA dataset with ALL available data sources...');
  ...
  // Read the 2023/2024 Excel extracted data
  const cutoffs2023_2024Path = path.join(rootDir, 'public', 'data', 'cutoffs-2023-2024.json');
  let cutoffs2023_2024 = { cutoffs: [] };
  
  try {
    if (fs.existsSync(cutoffs2023_2024Path)) {
      cutoffs2023_2024 = JSON.parse(fs.readFileSync(cutoffs2023_2024Path, 'utf8'));
      console.log(`Loaded 2023/2024 Excel data: ${cutoffs2023_2024.cutoffs.length} records`);
    }
  } catch (error) {
    console.error('Error reading 2023/2024 data:', error.message);
  }
  ...
  const allCutoffs = [...allProtoData, ...cutoffs2023_2024.cutoffs, ...cutoffs2025.cutoffs];
  
  console.log(`Total combined records: ${allCutoffs.length}`);
  
  // Create the final MEGA data structure matching proto.json exactly
  const finalData = {
    metadata: {
      last_updated: new Date().toISOString(),
      total_files_processed: 3, // proto.json + 2023/2024 Excel + 2025 Excel
      total_entries: allCutoffs.length,
      data_sources: ['proto.json (2023+2024)', 'Excel files (2023+2024)', 'Excel files (2025)'],
  ...
  // Save as MEGA dataset
  const megaFile = path.join(outDir, 'cutoffs-mega.json');
  fs.writeFileSync(megaFile, JSON.stringify(finalData, null, 2), 'utf8');
  ...
  console.log(`Data saved to: ${megaFile}`);
  ...
  if (allCutoffs.length < 200000) {
    ...
    console.log('1. Analyze remaining Excel file formats');
    ...
    // Calculate the exact number of entries over 200,000
    const excess = allCutoffs.length - 200000;
    console.log(`🎯 Exceeded target by ${excess.toLocaleString()} entries!`);
  }
}
