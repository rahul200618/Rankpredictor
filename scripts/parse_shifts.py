...
raw_data = """Username	Marks	Rank	10s1	10s2	10s3	25may
...
NeighborhoodLivid342	?	5.8k	✔			"""

def parse_rank(rstr):
    rstr = rstr.lower().replace('~', '').strip()
    rstr = rstr.replace('x', '0')
    if 'k' in rstr:
        num = rstr.replace('k', '').strip()
...
results = {'10s1': [], '10s2': [], '10s3': [], '25may': [], 'unknown': []}
shift_cols = ['10s1', '10s2', '10s3', '25may']
...
for line in lines[1:]:
    ...
    if len(parts) >= 3:
        try:
            marks = int(parts[1])
            rank = parse_rank(parts[2])
            if rank is None: continue
        except:
            continue
        
        # Determine shift by finding the '✔' character in the line
        shift = 'unknown'
        # The checkmark might be anywhere due to copy-paste. Let's just find where it is
        # but realistically, the headers were 10s1, 10s2, 10s3, 25may at columns idx 3, 4, 5, 6
        if '✔' in parts:
            idx = parts.index('✔')
            if idx == 3: shift = '10s1'
            elif idx == 4: shift = '10s2'
            elif idx == 5: shift = '10s3'
            elif idx >= 6: shift = '25may'
        else:
            # Maybe it uses generic text, let's see where the length varies
            pass
            
        results[shift].append({'marks': marks, 'rank': rank})

for k, v in results.items():
    print(f"Shift {k}: {len(v)} points")

with open('shifts_data.json', 'w') as f:
