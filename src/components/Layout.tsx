...
import {
  ...
  ChevronDown, Sparkles, Menu, X, LogOut, Shield
} from "lucide-react";
    ...
    <div className="flex flex-col h-screen bg-background overflow-hidden font-sans">
      ...
      >
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          <div className="relative w-8 h-8 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-blue-500/30 shrink-0 transition-transform group-hover:scale-110">
          ...
          <div className="hidden sm:block">
            <span className="font-extrabold text-base tracking-tight text-foreground leading-none flex items-center gap-1.5">
              PredictRank
              <Sparkles size={11} className="text-amber-400" />
            </span>
            <div className="text-[10px] text-muted-foreground font-medium leading-none mt-0.5">Karnataka Counseling Intelligence</div>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-1.5 px-3 py-1.5 rounded-2xl border border-border/40 bg-muted/40 backdrop-blur-md">
              ...
              >
                <Icon
                  ...
                  className={`transition-transform duration-300 group-hover:scale-110 ${
                    isActive ? "text-white animate-bounce-subtle" : "text-muted-foreground group-hover:text-foreground"
                  }`}
        ...
        {/* Exam toggle */}
        <div
          className="flex items-center gap-0.5 p-0.5 sm:p-1 rounded-xl border border-border bg-muted/60 backdrop-blur-sm shrink-0"
        ...
        <button
          ...
          onClick={toggleTheme}
          className="p-2 rounded-xl border border-border hover:bg-accent text-muted-foreground hover:text-foreground transition-all hover:scale-110"
        ...
        <button
          ...
          className="sm:hidden p-2 rounded-xl border border-border hover:bg-accent"
        >
          <Menu size={16} />
        </button>
      </header>
      ...
      {/* ── Main content ── */}
      <main id="main-scroll" className="flex-1 overflow-y-auto pb-10 sm:pb-16">
        {children}
      </main>
